...
for f in pub:
    ...
    for kw in ['function ScriptContext','ScriptContext(','class ScriptContext','api:{','api =','runOnNoteLoad','onNoteLoad','api.on('] :
        if kw in t:
            idxs=[m.start() for m in re.finditer(re.escape(kw), t)]
            for i in idxs[:3]:
                seg=t[max(0,i-150):i+250].replace(chr(10),' ')
                print(f'[{p}] {kw}:', seg[:400])
            break
