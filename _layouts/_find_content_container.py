...
for f in pub:
    if f[0]=='/public/src/content_renderer-BMc4Ylfn.js':
        ...
        # find note-content container class names
        for kw in ['note-content','ck-content','contenteditable','renderNote','note-detail','content-rendered']:
            idxs=[m.start() for m in re.finditer(re.escape(kw), t)]
            print(f'== {kw}: {len(idxs)}')
            for i in idxs[:3]:
                print('   ', t[max(0,i-80):i+120].replace(chr(10),' ')[:200])
        break
