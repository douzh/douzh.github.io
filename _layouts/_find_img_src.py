...
# look for image url resolution for text notes - search for 'api/attachments', 'attachment-image', 'resolveImageSrc', 'getImageSrc', 'processContent'
targets=['processContent', 'function processContent', 'attachment-image-', 'api/attachments/', 'getImageSrc', 'resolveAttach', 'data-type="image"']
for f in pub:
    p,off,sz=f
    ...
    for kw in targets:
        if kw in t:
            ...
            for i in idxs[:3]:
                seg=t[max(0,i-150):i+200].replace(chr(10),' ')
                print(f'[{p}] {kw}:', seg[:330])
            break
