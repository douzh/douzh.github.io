...
# find how frontend scripts are executed - search for 'frontendStartup' or injection of scripts into page
for f in pub:
    p,off,sz=f
    ...
    for kw in ['frontendStartup','triggerEvent(`activeNoteChanged`','triggerEvent("activeNoteChanged"','document.body`,`append','eval(`','new Function','injectScript','runFrontendScript','onNoteSwitched','noteSwitchedAndActivatedEvent']:
        if kw in t:
            idxs=[m.start() for m in re.finditer(re.escape(kw), t)]
            for i in idxs[:2]:
                seg=t[max(0,i-120):i+200].replace(chr(10),' ')
                print(f'[{p}] {kw}:', seg[:320])
            break
