...
# search for CORS / origin 403 logic
for kw in ['origin', 'Origin', '403', 'isLocalHost', 'referer', 'Referer', 'sameOrigin', 'CORS']:
    idxs=[m.start() for m in re.finditer(re.escape(kw), text)]
    if idxs:
        print(f'\n== {kw}: {len(idxs)} ==')
        for i in idxs[:4]:
            print('  ...', text[max(0,i-130):i+180].replace(chr(10),' ')[:300])
