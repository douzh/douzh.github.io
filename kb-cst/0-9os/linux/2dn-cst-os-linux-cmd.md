...
-   [common scene](id:dn-cst-linux-cmd-common-scene)
-   [lsof](id:dn-cst-linux-cmd-lsof)
-   [cmd ps](id:dn-cst-linux-cmd-ps)
...
- D 不可中断（收到信号不唤醒和不可运行，进程必须等待到有中断发生）
- R 运行（正在运行或者在运行队列d中等待）
- S 中断（休眠中，受阻，在等待某个条件的形成或接受到信号）
- T 停止（进程收到SIGSTOP, SIGSTP, SIGTIN, SIGTOU信号后运行停止）
...
lsof（List Open Files） 用于查看进程开打的文件，打开文件的进程，进程打开的端口(TCP、UDP)，找回/恢复删除的文件。
    ...
    lsof -d 6 |grep 6139  #查看某个进程使用某个文件描述符的情况
    lsof –i@10.65.64.23  #搜索某个地址打开的网络连接
    lsof –a +L1 /data  #寻找本地断开的打开文件
