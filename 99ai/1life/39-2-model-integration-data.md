...
前面章节按模型分类讲数据需求,本节从**存储角度**归纳:多模型集成系统需要几种存储类型,以及如何用最少存储支撑最多模型。

### 10.1 六种核心存储类型

| 存储类型 | 数据结构 | 支持的模型 | 典型技术 |
| --- | --- | --- | --- |
| **1. 文档存储** | 非结构化文本 | 32 RAG | Elasticsearch / MongoDB / 对象存储 |
| **2. 关系存储** | 结构化表格 | 32 SQL / 33 / 35(观测数据) | PostgreSQL / MySQL |
| **3. 图存储** | 三元组(节点-边-节点) | **32 符号 + 35 因果** | Neo4j / NebulaGraph |
| **4. 向量存储** | 高维嵌入向量 | 31 隐式检索 / 36 类比 | Pinecone / Chroma / FAISS |
| **5. 时序存储** | 时间序列 | 33 时序概率 / 监控 | InfluxDB / TimescaleDB |
| **6. 对象存储** | 二进制大文件 | 31 训练数据 / 34 几何模型 | S3 / MinIO |

### 10.2 关键洞察:图存储(三元组)统一符号与因果

**三元组可以同时支持符号模型和因果图**,这是存储层的关键简化。

#### 三元组的两种语义

| 用途 | 三元组形式 | 语义 | 示例 |
| --- | --- | --- | --- |
| 知识图谱(32 符号) | (实体,关系,实体) | 描述性关联 | (二甲双胍,治疗,糖尿病) |
| 因果图(35 因果) | (变量,因果作用,变量) | 方向性因果 | (用药,导致,康复) |

两者**结构同构**,都是图结构,差别在边的语义:
- 知识图谱的边:描述性关系(可对称可不对称)
- 因果图的边:方向性因果(严格有向无环 DAG)
...
// Neo4j 统一存储
// 知识图谱边
CREATE (m:Drug {name:"二甲双胍"})-[:TREATS]->(d:Disease {name:"糖尿病"})

// 因果图边(加属性区分)
CREATE (t:Treatment {name:"用药"})-[:CAUSES {type:"causal", confounded:true}]->(o:Outcome {name:"康复"})
```

**统一查询**:
```cypher
// 查询疾病的治疗药物(符号)
MATCH (m:Drug)-[:TREATS]->(d:Disease {name:"糖尿病"})
RETURN m.name

// 查询因果路径(因果)
MATCH path = (t:Treatment)-[:CAUSES*]->(o:Outcome)
RETURN path
```
...
| 符号 + 因果 | 两套系统 | 一套图数据库 |
| 一致性 | 需同步 | 天然一致 |
| 联合查询 | 难 | 易(符号查因果,因果查符号) |
| 维护成本 | 高 | 低 |

**案例**:医疗系统中,查"糖尿病的治疗药物"(符号)和"用药对康复的因果效应"(因果)可以在同一个图里完成——药物节点既是知识图谱实体,也是因果图的变量节点。
...
| 存储 | 职责 | 支持模型 |
| --- | --- | --- |
| 文档存储 | 原始文本、指南、论文 | 32 RAG |
| **图存储** | **知识图谱 + 因果图(统一)** | **32 符号 + 35 因果** |
| 关系存储 | 结构化业务数据、观测数据 | 32 SQL / 33 / 35(数据) |
| 向量存储 | 嵌入、相似检索 | 31 检索 / 36 类比 |
| 时序存储 | 传感器、监控、日志 | 33 时序 / 监控 |
| 对象存储 | 模型权重、图像、视频、CAD | 31 训练 / 34 几何 |

### 10.4 可简化的方向

#### 向量存储可合并进关系存储

现代数据库(PostgreSQL + pgvector)已支持向量检索,可合并:

| 方案 | 存储 |
| --- | --- |
| 分离 | 文档库(PDF)+ 向量库(嵌入) |
| **合并** | PostgreSQL(pgvector)同时存文本和向量 |

#### 时序存储可合并进关系存储

TimescaleDB 是 PostgreSQL 扩展,时序数据也可进关系库。

#### 极简方案:四种存储

| 极简存储 | 职责 | 技术 |
| --- | --- | --- |
| **关系存储** | 表格 + 时序 + 向量 | PostgreSQL + TimescaleDB + pgvector |
| **图存储** | 符号 + 因果 | Neo4j |
| **文档存储** | 原始文本 | Elasticsearch / 对象存储 |
| **对象存储** | 二进制大文件 | S3 / MinIO |
...
| 存储类型 | 推荐技术 | 备选 | 理由 |
| --- | --- | --- | --- |
| 关系存储 | PostgreSQL | MySQL | 生态成熟,扩展丰富 |
| 图存储 | Neo4j | NebulaGraph | 因果图查询强大,社区活跃 |
| 文档存储 | Elasticsearch | MongoDB | 全文检索强,RAG 友好 |
| 向量存储 | Chroma(小)/ Milvus(大) | Pinecone(托管) | 开源可控 |
| 时序存储 | TimescaleDB | InfluxDB | 基于 PG,统一栈 |
| 对象存储 | MinIO(自建)/ S3(云) | - | 标准 S3 协议 |

---

## 十一、一句话总结

> **各模型的数据需求差异巨大:符号模型(32)需非结构化文本(RAG,⭐)或结构化三元组(知识图谱,⭐⭐⭐⭐);物理仿真(34)需物理参数+几何模型 URDF/CAD(⭐⭐⭐);因果推理(35)需观测数据+因果图(因果图需专家构建,⭐⭐⭐⭐⭐最难);概率推理(33)需观测数据+先验分布(需领域知识,⭐⭐⭐);类比检索(36)需案例库(需积累,⭐⭐⭐);多智能体(37)需角色定义(⭐最易)。数据准备难度与模型"原生性"正相关——越原生精确的模型(因果、物理)数据越难准备,越隐式模糊的模型(RAG)门槛越低。这是大模型+RAG 最易落地、多模型全集成最难的根因,也是 [39](39-model-integration-engineering.md) L1→L2→L3 渐进路径的工程依据:从 RAG 起步,逐步引入更难准备的物理/因果/概率模型。完整多模型系统的数据流以医疗诊断为例:RAG(指南)+ 知识图谱(疾病关系)+ 影像(病灶)+ SQL(检验)+ 因果(治疗效应)+ 概率(疾病概率)+ 类比(历史病例)七种数据协同,输出结构化诊断结果。从存储角度归纳为六种核心类型(文档/关系/图/向量/时序/对象),其中图存储(三元组)是关键统一——同时支持符号模型(知识图谱)和因果图,一个 Neo4j 即可存两类边;极简方案四种存储(PostgreSQL+pgvector+TimescaleDB 合并表格/向量/时序 + Neo4j 符号+因果 + Elasticsearch 文档 + S3 对象)即可支撑完整多模型集成系统。**
